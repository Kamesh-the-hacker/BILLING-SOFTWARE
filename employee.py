...
class employeeclass:
    def __init__(self, root):
        self.root = root
        # Changed geometry to be relative to screen size and positioned to not cover the menu
        ...
        self.root.title("Employee Management | Saravana Fashion")
        ...
        # Search Frame - Adjusted width to fit screen
        SearchFrame = LabelFrame(self.root, text="Search Employee", font=("goudy old style", 12, "bold"), bd=2, relief=RIDGE, bg="#f0f0f0")
        ...
        txt_search = Entry(SearchFrame, textvariable=self.var_searchtxt, font=("goudy old style", 15), bg="#f0f0f0")
        txt_search.place(x=200, y=10, width=435)
        
        btn_search = Button(SearchFrame, text="Search", command=self.search, font=("goudy old style", 15), 
                          bg="#4caf50", fg="#f0f0f0", cursor="hand2")
        btn_search.place(x=650, y=10, width=130, height=30)

        # Title
        title = Label(self.root, text="Employee Details", font=("goudy old style", 15), bg="#0f4d7d", fg="#f0f0f0")
        title.place(x=50, y=100, width=window_width-100)

        # Content Area - Adjusted for proper scaling
        # Row 1
        lbl_empid = Label(self.root, text="Emp ID", font=("goudy old style", 15), bg="#f0f0f0").place(x=50, y=150)
        lbl_gender = Label(self.root, text="Gender", font=("goudy old style", 15), bg="#f0f0f0").place(x=350, y=150)
        ...
        lbl_dob = Label(self.root, text="D.O.B", font=("goudy old style", 15), bg="#f0f0f0").place(x=350, y=190)
        lbl_doj = Label(self.root, text="D.O.J", font=("goudy old style", 15), bg="#f0f0f0").place(x=750, y=190)

        txt_name = Entry(self.root, textvariable=self.var_name, font=("goudy old style", 15), bg="white")
        txt_name.place(x=150, y=190, width=180)
        txt_dob = Entry(self.root, textvariable=self.var_dob, font=("goudy old style", 15), bg="white")
        txt_dob.place(x=500, y=190, width=180)
        txt_doj = Entry(self.root, textvariable=self.var_doj, font=("goudy old style", 15), bg="white")
        ...
        lbl_pass = Label(self.root, text="Password", font=("goudy old style", 15), bg="#f0f0f0").place(x=350, y=230)
        lbl_utype = Label(self.root, text="User Type", font=("goudy old style", 15), bg="#f0f0f0").place(x=750, y=230)

        txt_email = Entry(self.root, textvariable=self.var_email, font=("goudy old style", 15), bg="white")
        txt_email.place(x=150, y=230, width=180)
        txt_pass = Entry(self.root, textvariable=self.var_pass, font=("goudy old style", 15), bg="white")
        ...
        lbl_salary = Label(self.root, text="Salary", font=("goudy old style", 15), bg="#f0f0f0").place(x=500, y=270)

        self.txt_address = Text(self.root, font=("goudy old style", 15), bg="white")
        self.txt_address.place(x=150, y=270, width=300, height=60)
        txt_salary = Entry(self.root, textvariable=self.var_salary, font=("goudy old style", 15), bg="white")
        txt_salary.place(x=600, y=270, width=180)

        # Buttons - Adjusted positioning and colors
        btn_add = Button(self.root, text="Save", command=self.add, font=("goudy old style", 15), 
                        bg="#2196f3", fg="#f0f0f0", cursor="hand2")
        ...
        # Employee Details Table - Adjusted for fullscreen
        emp_frame = Frame(self.root, bd=3, relief=RIDGE)
        emp_frame.place(x=0, y=350, relwidth=1, height=window_height-380)
        ...
        # Column widths
        for col in ("eid", "name", "email", "gender", "contact", "dob", "doj", "pass", "utype", "address", "salary"):
