...
class PackingSlipManager:
    ...
    def generate_unique_code(self):
        while True:
            code = 'PS' + ''.join(random.choices(string.ascii_uppercase + string.digits, k=6))
...
class PackingSlipViewer:
    def __init__(self):
        ...
        for col in columns:
            ...
            self.tree.column(col, width=int((window_width-60)/len(columns)))
    ...
    def generate_bill(self, slip_data):
        ...
        file = filedialog.asksaveasfilename(
            ...
            initialfile=f"{customer_name}_Invoice"
        )
        self.conn = sqlite3.connect('ims.db')
        self.cursor = self.conn.cursor()
    ...
    '''def create_invoice(self, total_with_gst, gst_amount, customer, total_amount, cart_data, file):
        try:
            ...
            c.drawString(width/2 - 50, height - 30, "e-Invoice")

            # Company Details
            c.setFont("Helvetica-Bold", 18)
            c.drawString(width/2 - 100, height - 80, "SARAVANA FASHION")
            
            c.setFont("Helvetica", 10)
            c.drawString(width/2 - 120, height - 100, "221/1, THILLAI NAGAR, FIRST STREET")
            c.drawString(width/2 - 100, height - 115, "DHARAPURAM ROAD, TIRUPUR - 641604")
            c.drawString(width/2 - 140, height - 130, "Phone No's : 86088 97777 / 90878 82233, E-Mail : kanizokids@gmail.com")
            c.drawString(width/2 - 80, height - 145, "GSTIN : 33DULPS8136R1ZL")
            c.drawString(width/2 - 90, height - 160, "State: TAMIL NADU, State Code: 33")

            # Customer Details (Left Side)
            c.setFont("Helvetica", 10)
            c.drawString(30, height - 200, f"M/s : {customer}")  # Customer name

            # Invoice Details (Right Side)
            current_date = datetime.now().strftime("%d-%m-%Y")
            invoice_no = random.randint(1000, 9999)
            c.drawString(400, height - 200, f"Invoice No : {invoice_no}")
            c.drawString(400, height - 215, f"Date : {current_date}")
            c.drawString(400, height - 230, "Transport : By Road")

            # Table Headers
            y_table = height - 300
            c.setFont("Helvetica-Bold", 10)
            headers = ["SI No", "HSN Code", "Description Of Goods", "Size", "Total Box", "Qty in Pcs", "Rate / Pcs", "Net Amount"]
            x_positions = [30, 80, 180, 300, 350, 420, 480, 530]
            
            for header, x in zip(headers, x_positions):
                c.drawString(x, y_table, header)

            # Draw lines
            c.line(20, y_table - 5, width - 20, y_table - 5)
            c.line(20, y_table + 15, width - 20, y_table + 15)
            ...
            for idx, item in enumerate(cart_data, 1):
                ...
                c.drawString(85, y_pos, "6109")  # HSN code
                c.drawString(185, y_pos, product_name)
                c.drawString(305, y_pos, str(size))
                c.drawString(355, y_pos, "1")  # Box count
                c.drawString(425, y_pos, str(qty))
                c.drawString(485, y_pos, f"{rate:.2f}")
                c.drawString(535, y_pos, f"{amount:.2f}")
            ...
            y_bottom = 150
            c.setFont("Helvetica-Bold", 10)
            c.drawString(350, y_bottom, f"GROSS AMT : {total_amount:.2f}")
            c.drawString(350, y_bottom - 20, f"IGST (5%) : {gst_amount:.2f}")
            c.drawString(350, y_bottom - 40, f"ROUND OFF : {round(total_with_gst) - total_with_gst:.2f}")
            c.drawString(350, y_bottom - 60, f"Bill Amount : {round(total_with_gst):.2f}")

            # Bank Details
            c.setFont("Helvetica", 9)
            c.drawString(30, 100, "Bank Details : SARAVANA FASHION")
            c.drawString(30, 85, "Bank Name : INDIAN BANK")
            c.drawString(30, 70, "Account No : 6372272999")
            c.drawString(30, 55, "Branch : TIRUPUR")
            c.drawString(30, 40, "IFSC Code : IDIB0001109")
            c.drawString(30, 25, "Payment Mode : NEFT / RTGS / UPI")

            # Footer
            c.setFont("Helvetica-Bold", 8)
            c.drawString(30, 10, "E & O E")
            c.drawString(width - 200, 10, "For SARAVANA FASHION")
            c.drawString(width - 200, 40, "Authorized Signatory")
    ...
    def generate_pdf(self,file,cart_data,customer,cs):
        ...
        # Logo placeholder
        c.rect(10*mm, height-35*mm, 30*mm, 25*mm, fill=0)
        ...
        c.drawString(20*mm, height-55*mm, f"{cs[9]}")
        ...
        # Table content
        data = [
            ["1", "6109", "BOYS T SHIRT (WHITE ADDAS)", "1, 2, 3", "34", "1700", "12", "20400"],
            ["2", "6109", "BOYS T SHIRT (MIX ADDAS)", "1,2,3", "1", "30", "12", "360"]
        ]
        c.setFont("Helvetica", 8)
        for row_num, row in enumerate(data):
            y_pos = table_top-11*mm - row_num*5*mm
            for i, item in enumerate(row):
                #c.drawString(x_positions[i]*mm + 1*mm, y_pos, item)
                pass
        ...
        total_qty = 0
        y_pos = table_top-4.7*mm - row_num*5*mm
        for idx, item in enumerate(cart_data, 1):
            product_name, size, _, qty, amount = item
            rate = amount / qty if qty else 0
                
            c.setFont("Helvetica", 9)
            c.drawString(27, y_pos, str(idx))
            c.drawString(45, y_pos, "6109")  # HSN code
            c.drawString(105, y_pos, product_name)
            c.drawString(265, y_pos, str(size))
            c.drawString(325, y_pos, "1")  # Box count
            c.drawString(490, y_pos, str(qty))
            c.drawString(445, y_pos, f"{rate:.2f}")
            c.drawString(515, y_pos, f"{amount:.2f}")
        ...
        # Totals
        c.setFont("Helvetica-Bold", 9)
        c.drawString(15*mm, table_bottom-4*mm, "Total :")
        c.drawString(115*mm, table_bottom-4*mm, "35")
        c.drawString(135*mm, table_bottom-4*mm, "1730")
        c.drawString(151*mm, table_bottom-4*mm, "GROSS AMT :")
        c.drawString(175*mm, table_bottom-4*mm, "20760")
        ...
        # Amount details
        c.drawString(120*mm, 70*mm, "IGST")
        c.drawString(180*mm, 70*mm, "1038.00")
        c.drawString(120*mm, 65*mm, "ROUND OFF")
        c.drawString(180*mm, 65*mm, "0")
        c.setFont("Helvetica-Bold", 9)
        c.drawString(120*mm, 60*mm, "Bill Amount :")
        c.drawString(180*mm, 60*mm, "21798")

        # Amount in words
        c.setFont("Helvetica-Bold", 9)
        c.drawString(10*mm, 40*mm, "Amount In Words :")
        c.drawString(40*mm, 40*mm, "Twenty One Thousand Seven Hundred and Ninty Eight only")
        c.line(5*mm, height-260*mm, width-5*mm, height-260*mm)
        ...
        c.save()
        print("Perfect invoice PDF generated successfully!")
class BillClass:
    def __init__(self, root):
        ...
        title = Label(product_frame, text="Generate Packing Slip", font=("goudy old style", 18), bg="#404a42", fg='white').pack(side=TOP, fill=X)
        ...
        Button(product_frame, text="View Packing Slips", command=self.open_packing_slip_viewer, font=("goudy old style", 15), bg="#2196f3", fg="white", cursor="hand2").place(x=300, y=520, width=130, height=40)
        self.update_pro(None)


        self.cart_listbox = Listbox(self.root, height=39, width=130)
        self.cart_listbox.place(x=500, y=20)
        self.cart_listbox.insert(tk.END, f'        Product                                                     Size                                      Quantity                                           Price                                         Total')
    ...
    def fetch_pro(self, category_name):
        with sqlite3.connect("ims.db") as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT name FROM product WHERE category = ?", (category_name,))
            pro = cursor.fetchall()
    ...
    def fetch_bi(self):
        with sqlite3.connect("ims.db") as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT DISTINCT agencyname FROM kyc")
            kyc = cursor.fetchall()
    ...
    def add_to_cart(self):
        ...
        selected_size = self.cmb_si.get()
        selected_price = self.price_list_combo.get()  # Fetch selected price directly
        ...
        try:
            ...
            price = float(selected_price)  # Parse selected price to float
        except ValueError:
            messagebox.showerror("Error", "Quantity and price must be numbers!")
            return

        total_price = price * quantity
        self.cart.append((product_name, selected_size, selected_price, quantity, total_price))
        self.cart_listbox.insert(tk.END, f"       {product_name}                          ({selected_size})                                     {quantity}                                                    {price}                                                  {total_price}")
    ...
    def generate_ps(self):
        ...
        self.name = self.cmb_bi.get()
        self.cursor.execute("SELECT * FROM kyc WHERE agencyname=?", (self.name,))
        customer = self.cursor.fetchone()
    ...
    def create_ps(self, mypath, customer, code):
        ...
        # Use 6x4 inch page size (convert to points)
        ...
        # Draw size column headers with smaller width
        sizes = ["75", "80", "85", "90", "95", "100", "105", "110", "TOTAL"]
        x_pos = 110
        for size in sizes:
            c.rect(x_pos, y_table, 35, 15)
            c.drawString(x_pos + 3, y_table + 5, size)
            x_pos += 35
        ...
        for product_name, sizes in product_groups.items():
            ...
            x_pos = 110
            total = 0
            for size in ["75", "80", "85", "90", "95", "100", "105", "110"]:
        ...
        # Draw footer
        y_pos -= 30
        c.line(10, y_pos, 100, y_pos)  # Line for prepared by
        c.line(page_width-100, y_pos, page_width-10, y_pos)  # Line for checked by
        
        c.setFont("Helvetica", 8)
        c.drawString(30, y_pos - 10, "ADMIN2")
        c.drawString(page_width-80, y_pos - 10, "THANGAPAL.R")
        
        c.drawString(25, y_pos - 20, "Prepared By")
